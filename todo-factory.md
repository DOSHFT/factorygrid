...
- [x] P0: Lock network exposure. Compose now defaults non-public services to loopback, limits approved LAN surfaces to RuFloUI frontend `28589` and LiteLLM published gateway `4001`, adds `bin/factory-check-network-exposure.sh`, and restores `bin/factory-expose-lan.ps1` for explicit Windows portproxy setup.
- [ ] P0: Add per-agent workspace guardrails: git snapshot before each run, allowlisted write paths, config-file HITL gate, and rollback instructions. Progress: RuFloUI deterministic bounded file-write tasks now enforce the write allowlist, refuse protected paths, create pre-write snapshots, and return rollback instructions.
