    ...
    >
      ...
      {taskOutput.length > 0 && (isRunning || task.status === 'failed' || (expanded && isDone)) && (
        <div onClick={(e) => e.stopPropagation()} style={{
          ...
          border: `1px solid ${task.status === 'failed' ? 'var(--accent-red)' : 'var(--border)'}`, height: 200,
...
export default function TasksPanel() {
  ...
  useEffect(() => {
    ...
    const handleWsMessage = createBufferedMessageHandler((msg: { type: string; payload?: any }) => {
      ...
      if ((msg.type === 'task:updated' || msg.type === 'task:added') && msg.payload) {
        ...
        if (t.id) wsUpdatedRef.current.set(t.id, { task: t, ts: Date.now() })
      }
