...
import { createBufferedMessageHandler } from '@/utils/wsBatcher'
...
function AgentOutputModal({ agent, onClose }: { agent: SwarmAgent; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const handleWsMessage = createBufferedMessageHandler((msg: { type: string; payload: { agentId: string; line: string } }) => {
      if (msg.type === 'agent:output' && msg.payload.agentId === agent.id) {
        setLines(prev => [...prev, msg.payload.line].slice(-1000))
      }
    }, { flushMs: 75, maxBatchSize: 50 })
    ws.onmessage = (event) => {
        ...
        >
          {loading ? (
            ...
            <Virtuoso
              ...
              data={lines}
              followOutput="auto"
              itemContent={(i, line) => formatLine(line, i)}
            />
          )}
        </div>
...
export default function SwarmMonitorPanel() {
  ...
  useEffect(() => {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
    const ws = new WebSocket(`${protocol}//${window.location.host}/ws`)
    const handleWsMessage = createBufferedMessageHandler((msg: { type: string; payload: unknown }) => {
      ...
      setOutputAgent(prev => {
        ...
        return { ...prev, status: activity.status as SwarmAgent['status'], currentAction: activity.currentAction, currentTask: activity.currentTask }
      })
    }, { flushMs: 75, maxBatchSize: 50 })

    ws.onmessage = (event) => {
      try { handleWsMessage(JSON.parse(event.data)) } catch { /* ignore bad messages */ }
    }
