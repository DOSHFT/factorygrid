...
function renderActionResult(result: unknown): string {
  ...
  if (typeof record.raw === 'string' && Object.keys(record).length <= 2) return record.raw.replace(/\r/g, '')
  if (Array.isArray(record.bottlenecks)) {
    return [
      String(record.summary || 'Bottleneck report'),
      '',
      ...record.bottlenecks.map((item) => {
        const row = item as Record<string, unknown>
        return `${row.component} [${row.severity}]\nFinding: ${row.finding}\nSolution: ${row.solution}`
      }),
    ].join('\n\n')
  }
